...
@st.cache_data
def get_video_summary(df):
    """缓存视频维度的汇总信息，避免每次切换视频都重新 Groupby"""
    return df.groupby(["bvid", "title", "main_category"]).agg(
...
if menu == "大盘宏观追踪":
    if len(df_latest) > 0 and "rank_date" in df_latest.columns:
        ...
        with tab1:
            ...
            if not pivot.empty:
                fig_line = px.line(pivot, title="各一级分区每日上榜视频数量趋势", labels={"value": "上榜数量", "variable": "一级分区", "rank_date": "日期"}, markers=True)
                fig_line.update_layout(height=450, hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02))
        ...
        with tab2:
            ...
            fig_tree = px.treemap(cat_counts, path=[cat_col], values="count", title="各一级分区上榜视频总数结构分布", color="count", color_continuous_scale="blues")
    ...
    if len(df_latest) > 0 and "engage_rate" in df_latest.columns:
        ...
        with col_left:
            ...
            if "hours_since_pub" in df_latest.columns: hover_data_cols.append("hours_since_pub")
            fig_scatter = px.scatter(
                df_latest, x="view_count", y="engage_rate", color=cat_col, size="coin_count", hover_data=hover_data_cols,
                title="三连互动率 vs 播放量（气泡大小 = 投币数）", labels={"view_count": "播放量", "engage_rate": "三连互动率", cat_col: "一级分区"}, log_x=True,
            )
            median_engage = df_latest["engage_rate"].median()
            median_view = df_latest["view_count"].median()
            fig_scatter.add_hline(y=median_engage, line_dash="dash", line_color="gray", annotation_text=f"互动率中位数: {median_engage:.4f}")
    ...
    if len(df_latest) > 0:
        ...
        with tab_wc:
            if not JIEBA_AVAILABLE: st.warning(" 缺少 jieba 库，请执行 pip install jieba 启用分词功能")
            ...
            if freq:
                ...
                fig_bar_wc = px.bar(words_df, x="count", y="word", orientation="h", title="标题高频词 Top 30", color="count", color_continuous_scale="viridis")
        ...
        with tab_dur:
            if "duration_bucket" in df_latest.columns:
                ...
                with col1:
                    fig_dur = px.bar(dur_stats, x="duration_bucket", y="avg_views", color="duration_bucket", title="各时长区间平均播放量", text_auto=",.0f")
                    fig_dur.update_layout(showlegend=False, height=400)
                    st.plotly_chart(fig_dur, use_container_width=True)
                with col2:
                    fig_eng = px.bar(dur_stats, x="duration_bucket", y="avg_engage", color="duration_bucket", title="各时长区间平均互动率", text_auto=".4f")
    ...
    has_timeseries = ("snapshot_seq" in df_filtered.columns and df_filtered["snapshot_seq"].notna().any() and df_filtered["snapshot_seq"].max() > 1)
    if not has_timeseries:
        ...
        if search_kw.strip():
            mask = video_list["title"].str.contains(search_kw, case=False, na=False) | video_list["bvid"].str.contains(search_kw, case=False, na=False)
        ...
        if filtered_video_list.empty:
            ...
            selected_bvid = st.selectbox(
                "选择目标视频进行深度下钻:", options=video_options,
                format_func=lambda bv: next((f"[{r['main_category']}] {r['title'][:50]}... ({r['bvid']}) | 记录{int(r['snapshot_count'])}期" for _, r in filtered_video_list.iterrows() if r["bvid"] == bv), bv),
                index=0,
            )
            ...
            if len(video_df) < 2:
                ...
                with kpi1: st.metric(" 峰值流速", f"{peak_v:,}", delta=f"@{peak_time_str}" if peak_v > 0 else None)
                with kpi2: st.metric(" 上榜总涨幅", f"{total_growth:,}")
                with kpi3:
                    if "hours_since_pub" in video_df.columns: st.metric(" 上榜已发布时长", f"{video_df['hours_since_pub'].iloc[-1]:.1f} h")
                    else: st.metric(" 记录期数", f"{len(video_df)} 期")
                with kpi4: st.metric(" 初期互动率", f"{first_engage:.4f}")
                with kpi5: st.metric(" 互动衰减", f"{engage_change:+.4f}", delta_color="inverse" if engage_change < 0 else "normal")
                ...
                with track_col1:
                    if "hours_since_pub" in video_df.columns:
                        ...
                        x_col, x_label = "snapshot_time", "快照时间"

                    fig_line_track = px.line(video_df, x=x_col, y="view_count", title=f"「{video_title[:25]}...」生命周期爆发曲线", labels={x_col: x_label, "view_count": "累计播放量"}, markers=True)
                ...
                with track_col2:
                    ...
                    fig_single_funnel = px.funnel(funnel_data, x='数量', y='阶段', title="当前视频最新累计转化漏斗")
                ...
                with st.expander(" 查看逐期增量详情"):
                    detail_cols = ["snapshot_time", "hours_since_pub", "view_count", "delta_views", "delta_likes", "delta_coins", "engage_rate", "engagement_decay", "rank_position"]
                    st.dataframe(
                        video_df[[c for c in detail_cols if c in video_df.columns]].reset_index(drop=True),
...
# ====== 底部通栏 ======
st.markdown("---")
st.caption(f"数据来源: {DEFAULT_FILE} | 更新时间: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}")
